<template>
  <main class="layout">
    ...
    <div class="layout__content">
      ...
      <div
        v-if="!isLoading && filteredOrders.length === 0 && orders.length > 0"
        class="layout__empty"
      >
        ...
        >
          <div class="order__wrapper">
            ...
            <div class="order__sum">
              <span
                >Сумма заказа:
                {{ getOrderTotal(order).toLocaleString("ru-RU") }} ₽</span
              >
            </div>
          ...
          <ul
            v-if="order.orderPizzas && order.orderPizzas.length > 0"
            class="order__list"
          >
            <li
            ...
            >
              <div class="product">
                <div class="product__img product__img--pizza">
                  <div
                    :class="`pizza pizza--foundation--${getOrderPizzaSizeKey(pizza)}-${getOrderPizzaSauceKey(pizza)}`"
                  >
                    <div class="pizza__wrapper">
                      <div
                        v-for="(
                          ingredient, index
                        ) in getOrderPizzaIngredientsKeys(pizza)"
                ...
                <div class="product__text">
                  ...
                  <ul>
                    <li
                      v-for="(part, index) in formatPizzaDescription(pizza)"
                      :key="index"
                    >
                      {{ part }}
                    </li>
              ...
              <p class="order__price">
                <span v-if="pizza.quantity > 1">{{ pizza.quantity }}×</span
          ...
          >
            <li v-for="misc in order.orderMisc" :key="misc.id">
              <img
                :src="getMiscImageUrl(misc.miscId)"
                width="20"
                height="30"
                :alt="dataStore.getMiscById(misc.miscId)?.name || ''"
              />
              <p>
                <span>{{
                  dataStore.getMiscById(misc.miscId)?.name || "Неизвестно"
                }}</span>
                <b>
                  <span v-if="misc.quantity > 1"
                    >{{ misc.quantity }}×{{
                      dataStore.getMiscById(misc.miscId)?.price || 0
                    }}=</span
                  >{{
                    (dataStore.getMiscById(misc.miscId)?.price || 0) *
                    misc.quantity
          ...
          <p class="order__address">
            Адрес доставки: {{ formatAddress(order) }}
          </p>
        </section>
      </TransitionGroup>
    </div>
  ...
  if (sauce && dough && size) {
    ...
    if (pizza.ingredients) {
      pizza.ingredients.forEach(
        (ing: { ingredientId: number; quantity: number }) => {
...
const getOrderPizzaIngredientsKeys = (
  pizza: OrderPizza,
...
const repeatOrder = (order: Order) => {
  if (
    !order.orderPizzas &&
    (!order.orderMisc || order.orderMisc.length === 0)
  ) {
  ...
  if (order.orderPizzas && order.orderPizzas.length > 0) {
    order.orderPizzas.forEach((pizza: OrderPizza) => {
      ...
      if (!dough || !size || !sauce) {
        logger.warn(
          `Не найдены данные для пиццы: dough=${pizza.doughId}, size=${pizza.sizeId}, sauce=${pizza.sauceId}`,
        );
        return;
      }
